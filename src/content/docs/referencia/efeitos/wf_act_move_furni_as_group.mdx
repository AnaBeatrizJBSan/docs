---
title: "Efeito WIRED: Mover Móbis em Grupo"
description: Este efeito move um grupo de mobis para um local de destino; a distância entre os mobis permanecerá consistente.
sidebar:
  label: Mover Móbis em Grupo
  badge:
    text: RASCUNHO
    variant: note
infobox:
  # == Infobox ==
  # hide: false # valor padrão
  title: Mover Móbis em Grupo
  # image: # url preenchida automaticamente

  # == Dados do Mobi ==
  revision: 75129
  classname: wf_act_move_furni_as_group
  name: "Efeito WIRED: Mover Móbis em Grupo"
  description: Este efeito move um grupo de mobis para um local de destino; a distância entre os mobis permanecerá consistente.
  # product_name: # caso o nome na loja seja diferente do nome do Mobi
  # image_direction: 0 # valor padrão
  # image_animated_state: 100 # valor padrão
  # icon: # url preenchida automaticamente
  availability: No catálogo e CA
  # price: # preço em créditos no catálogo
  release_date: 2026-05-12

  # == Dados do Wired ==
  type: Efeito
  # requires_bot: false # valor padrão
  requires_furni: true
  # requires_antena: false # valor padrão
  # requires_contract: false # valor padrão
  # requires_chest: false # valor padrão
  # execution_limit: 100 # valor padrão
  # execution_limit_per_user: 100 # valor padrão

  # == Dados do Efeito ==
  additional_sources: []
---

import Infobox from "@components/Infobox.astro"

<Infobox />
