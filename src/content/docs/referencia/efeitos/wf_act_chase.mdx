---
title: "EFEITO WIRED: Perseguir"
description: "Este efeito move o mobi para o usuário mais próximo dele na mesma linha ou coluna. Se tiver um usuário próximo a este mobi, o ATIVADOR WIRED: Batida irá ativar."
sidebar:
  label: Perseguir
  badge:
    text: RASCUNHO
    variant: note
infobox:
  # == Infobox ==
  # hide: false # valor padrão
  title: Perseguir
  # image: # url preenchida automaticamente

  # == Dados do Mobi ==
  revision: 50950
  classname: wf_act_chase
  name: Perseguir
  description: "Este efeito move o mobi para o usuário mais próximo dele na mesma linha ou coluna. Se tiver um usuário próximo a este mobi, o ATIVADOR WIRED: Batida irá ativar."
  # product_name: # caso o nome na loja seja diferente do nome do Mobi
  # image_direction: 0 # valor padrão
  # image_not_animated: false # valor padrão
  # image_animated_state: 100 # valor padrão
  # icon: # url preenchida automaticamente
  availability: No catálogo e CA
  price: 7
  # release_date: # data de lançamento

  # == Dados do Wired ==
  type: Efeito
  # requires_bot: false # valor padrão
  requires_furni: true
  # requires_antena: false # valor padrão
  # requires_contract: false # valor padrão
  # requires_chest: false # valor padrão
  # execution_limit: 100 # valor padrão
  # execution_limit_per_user: 100 # valor padrão

  # == Dados do Efeito ==
  # negative_version: # se aplicável
  # name: # nome para exibição
  # description: # descrição para exibição
  # revision: # revisão técnica
  # classname: # classname técnico
---

import Infobox from "@components/Infobox.astro"

<Infobox />
