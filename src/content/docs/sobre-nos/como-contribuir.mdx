...
Para uma correção pequena em uma página, use o link **Editar página** no rodapé. Para editar múltiplos arquivos diretamente no navegador sem instalar nada, use o **VS Code Web** pressionando a tecla **`.`** no repositório. Para criar novas páginas, componentes ou testar o site em tempo real, trabalhe em seu computador.
</Aside>
...
O **VS Code Web** (disponível via [github.dev](https://github.dev)) permite editar arquivos do projeto utilizando a interface completa do Visual Studio Code diretamente pelo navegador, sem necessidade de clonar o repositório ou instalar ferramentas locais.

<Aside type="tip" title="Como abrir o VS Code Web">
Ao navegar em qualquer página do repositório no GitHub (inclusive em seu *fork*), basta pressionar a tecla **`.`** (ponto) no teclado. O repositório abrirá instantaneamente no editor web. Você também pode acessar trocando `github.com` por `github.dev` na barra de endereço do navegador.
</Aside>
...
<Aside type="note" title="Limitação do ambiente Web">
O VS Code Web roda exclusivamente dentro do navegador e não inclui um terminal local para executar `npm run dev` ou `npm run build`. Caso sua contribuição envolva componentes interativos complexos ou alterações nas configurações do Astro/Starlight, é recomendado testar em seu computador.
</Aside>
...
<Aside type="tip" title="Quando fazer isso?">
Se você começou uma contribuição há alguns dias, ou se percebe que o projeto recebeu várias alterações desde então, sincronize seu fork antes de abrir o pull request. Para contribuições que demoram mais tempo, vale a pena repetir esse processo regularmente.
</Aside>
...
Se tiver uma ideia, dúvida ou encontrar um problema, abra uma [issue no GitHub](https://github.com/WiredClub/docs/issues). Para conversar com a comunidade, entre no [Discord do Wired Club](https://discord.wiredclub.com.br).
