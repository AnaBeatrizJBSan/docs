export const getFurniIcon = (classname: string, revision: number) => {
	return `https://images.habbo.com/dcr/hof_furni/${revision}/${classname.replace('*', '_')}_icon.png`;
};

export const getFurniImage = (classname: string, revision: number, dir: number = 0, state: number = 0) => {
	return `https://habcat.net/media/furni2/${revision}/${classname.replace('*', '_')}/${dir}_${state}.webp`;
};
